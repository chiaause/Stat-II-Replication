...
# count the number of NA values of margin
sum(is.na(rdd_data$margin))
...
medium <- population_density %>% filter(subsample == "Medium")
high <- population_density %>% filter(subsample == "High")

diff_pop_density <- bind_cols(low %>% select(dv, estimate, se),
                              medium %>% select(estimate, se),
                              high %>% select(estimate, se))

names(diff_pop_density)  <- c("Outcome", rep(c("Estimate", "SE"), 3))
...
kableExtra::kable(diff_pop_density, booktabs = T,
  ...
  kableExtra::add_header_above(c(" " = 1, "Low" = 2, "Medium" = 2,"High" = 2), bold = T) %>%
  kableExtra::kable_paper(full_width = T) %>%
  kableExtra::kable_classic() %>%
  kableExtra::kable_styling(font_size = 11) 
